	...
	if(mode == 1)
		...
		if(length(damaged) > 0)
			for(var/obj/item/organ/external/org in damaged)
				. += "<span class='info'>&emsp;[capitalize(org.name)]: [(org.brute_dam > 0) ? "<font color='red'>[org.brute_dam]</font></span>" : "<font color='red'>0</font>"]-[(org.burn_dam > 0) ? "<font color='#FF8000'>[org.burn_dam]</font>" : "<font color='#FF8000'>0</font>"]"
/*
	if(H.status_flags & FAKEDEATH)
...
*/
	...
	for(var/thing in H.viruses)
		...
		if(!(D.visibility_flags & HIDDEN_SCANNER))
			...
			. += "&emsp;Возможное лечение: [D.cure_text]</span>"
	if(H.undergoing_cardiac_arrest())
		...
		if(heart && !(heart.status & ORGAN_DEAD))
			...
			. += "&emsp;Стадия: 1/1"
			. += "&emsp;Возможное лечение: Электрический шок</span>"
